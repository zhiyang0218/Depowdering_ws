...
def callback(cloud_msg):
    ...
    filename = os.path.join(output_dir, 'point_cloud_snapshot.csv')
    
    with open(filename, 'w') as csvfile:  # Changed from 'a' to 'w' (overwrite)
        writer = csv.writer(csvfile)
        writer.writerow(['x', 'y', 'z', 'r', 'g', 'b', 'timestamp'])  # Always write header
    ...
    rospy.signal_shutdown("Snapshot captured")  # Stop after first message
